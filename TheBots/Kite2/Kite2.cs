...
namespace Kite2
{
  ...
  private void firing_solution(Events events)
  {
    if (events.RobotsScanned.Count>0)
    {
        var last = (_target_positions.Count > 0) ? _target_positions[_target_positions.Count-1] : new Vector(0, 0);
      ...
      foreach(var d in events.RobotsScanned)
      {
        ...
        if (robotsScanned.Count > 0)
        {
            ...
            foreach (var distance in events.RobotsScanned) if (distance<_min_distance) _min_distance = distance;
        }
    }
